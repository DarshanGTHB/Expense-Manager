...
function ExpenseForm({ userId,counter,result,setResult }) {
    ...
    const handleSubmit = () => {
        ...
        if (result !== 'Enter proper data' && result!=='Insufficient Balance'){
            formData.account = ''
            formData.amount = ''
            formData.title = ''
    ...
    return (<>
        <div className='record-form'>
            ...
            <button className='record-btn' placeholder='Recieved From' value='Submit' onClick={handleSubmit} ><img width='40px' src={sub} alt='submit' /></button>
        </div>
        <div className='remark' style={{ color: (result === 'Enter proper data' || result==='Insufficient Balance') ? 'red' : '#6cc9ff' }}>{result}</div></>
    )
}
