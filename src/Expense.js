...
function Expense({ counter, exTotal, userId, result }) {
    ...
    useEffect(() => {
        fetch('/api', {
            ...
            body: JSON.stringify("SELECT E.TRANSICTION_ID,E.TITLE,E.AMOUNT,D.ACCOUNT_NAME,E.DESCRIPTION,E.CATEGORY,U.USER_NAME FROM DEFAULT_EXPENSE E JOIN DEFAULT_ACCOUNT D ON E.ACCOUNT_ID=D.ACCOUNT_ID AND E.USER_ID=D.USER_ID LEFT JOIN USER_DATA U ON E.SENT_TO=U.USER_ID WHERE to_char(E.TRANSICTION_DATE) = to_char(SYSDATE+" + counter + ") AND E.USER_ID='" + userId + "' ORDER BY E.TRANSICTION_DATE"),
    ...
    return (
        <div id='expense-box'>
            ...
            <div className='record-list'>{(exData===undefined) ? <div className='no-record'>Unable to fetch data!</div> : (exData.length>0)? exData.map((row) => {
                
                return (
                    <div key={row[0]} className='record'>
                        <div className='record-left'>
                            <span className='record-first-line'>
                                <img src={require(`./img/${(row[5]!==undefined)?row[5]:'IC0'}.png`)} alt={row[5]} className='record-img' />
                        ...
                        <div>
                            ...
                            <div className='record-to'>{(row[6]!=null)?'to '+row[6]:''}</div>
                        </div>
                    </div>
                )
