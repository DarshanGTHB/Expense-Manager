...
function Income({counter,inTotal,userId,result}) {
    ...
    return (
        <div id='income-box'>
            ...
            <div className='record-list'>{(inData===undefined) ? <div className='no-record'>Unable to fetch data!</div> : (inData.length>0)?inData.map((row)=>{
                return (
                    <div key={row[0]} className='record'>
                        <div className='record-left'>
                            <span className='record-first-line'>
                                <img src={require(`./img/${(row[5]!==undefined)?row[5]:'IC0'}.png`)} className='record-img' alt={row[5]}/>
